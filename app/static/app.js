  ...
  function renderPlayers() {
    ...
    $("#playerList").innerHTML =
      filtered
        .map(
          ({ p, i }) =>
            `<div class="player-item ${i === state.playerIndex ? "active" : ""}" data-player-index="${i}">${p.avatar_url ? `<img class="player-avatar player-avatar-photo" src="${esc(p.avatar_url)}" alt="${esc(p.player_id)}" loading="lazy">` : `<div class="player-avatar">${esc(String(p.jersey_number || p.global_ids?.[0] || "?"))}</div>`}<div><b>${esc(p.display_id || p.player_id)}</b><span>${esc(p.team)} · ${p.global_ids.length} 个碎片 ID 已合并</span></div><span class="player-distance">${(p.total_distance_m / 1000).toFixed(2)}km</span></div>`,
        )
