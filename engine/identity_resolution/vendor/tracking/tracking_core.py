# -*- coding: utf-8 -*-
"""tracking V3 的检测、局部跟踪、全局重关联、渲染与候选事件核心。

统一帧域：Stage 1～4 的分析数据全部使用处理帧编号 ``proc_idx``；
``vid_stride`` 只用于原视频 I/O 与处理帧率换算。最终包装层默认保留所有达到
最低出场要求的候选簇，不以16人先验硬截断global ID。
"""
...
from tracking_lib.team_features import aggregate_features, torso_feature
...
def cuda_bf16_iter(iterator, device):
    """Run CUDA model forwards in BF16 AMP; fall back to FP16 if BF16 is unavailable."""
    enabled = str(device) != "cpu" and torch.cuda.is_available()
...
class SportsOSNetReIDExtractor:
    """SportsMOT-trained OSNet features for cross-tracklet association.

    ``reid_root`` is the Deep-EIoU checkout directory containing its ``reid``
    package.  Loading this legacy, locally trusted checkpoint requires the
    PyTorch compatibility flag only while the extractor is constructed.
    """

    def __init__(self, reid_root, model_path, device="cuda:0", batch_size=128):
        root = Path(reid_root).resolve()
        model_path = Path(model_path).resolve()
        if not (root / "reid").is_dir() or not model_path.is_file():
            raise FileNotFoundError(
                f"Sports OSNet source/checkpoint missing: {root / 'reid'} / {model_path}"
            )
        if not str(device).startswith("cuda") or not torch.cuda.is_available():
            raise RuntimeError("Sports OSNet production profile requires CUDA")
        ...
        try:
            base = FeatureExtractor(
                model_name="osnet_x1_0", model_path=str(model_path), device=str(device)
            )
        finally:
            if previous is None:
        ...
        self.preprocess = base.preprocess
    ...
    def __call__(self, crops):
        ...
        for offset in range(0, len(crops), self.batch_size):
            tensors = []
            for crop in crops[offset:offset + self.batch_size]:
                if crop.size == 0:
...
def per_box_max_iou(xyxy):
    """Return each box's largest IoU with another box in the same frame."""
...
def stage1_detect_track(args, reid):
    ...
    tracklets = defaultdict(lambda: {
        ...
        "col_cnt": 0,
        "appearance_samples": [],
        "reid_overlap_skips": 0,
        "team_feature_samples": [],
        "team_feature": np.zeros(198, dtype=np.float32),
    ...
    results = model.track(
        source=args.video,
        classes=[0, 32],
        tracker=getattr(args, "tracker_config", "botsort.yaml"),
        persist=True,
        stream=True,
        conf=args.conf,
        iou=0.5,
    ...
    frame_idx = -1
    for r in cuda_bf16_iter(results, args.device):
        frame_idx += 1
        frame = r.orig_img
        ...
        person_indices = [i for i in range(len(cls)) if cls[i] == 0 and ids[i] >= 0]
        person_overlaps = per_box_max_iou(xyxy[person_indices])
        overlap_by_index = {
            source_index: float(person_overlaps[index])
            for index, source_index in enumerate(person_indices)
        }
        for i in range(len(cls)):
            ...
            tk["last"] = frame_idx
            if do_reid and overlap_by_index.get(i, 0.0) < args.reid_max_iou:
                x1i, y1i = max(0, int(x1)), max(0, int(y1))
                crop = frame[y1i:int(y2), x1i:int(x2)]
                crops.append(crop)
                crop_ids.append(tid)
            elif do_reid:
                tk["reid_overlap_skips"] += 1
        ...
        if crops:
            ...
            for k, tid in enumerate(crop_ids):
                tk = tracklets[tid]
                if tk["emb_cnt"] == 0:
                    tk["emb_sum"] = np.zeros_like(embs[k], dtype=np.float32)
                tk["emb_sum"] += embs[k]
                tk["emb_cnt"] += 1
                color = color_feature(crops[k])
                tk["col_sum"] += color
                tk["col_cnt"] += 1
                tk["appearance_samples"].append((frame_idx, embs[k], color))
                crop = crops[k]
                team_item = torso_feature(crop, (0, 0, crop.shape[1], crop.shape[0]))
                if team_item is not None:
                    tk["team_feature_samples"].append((frame_idx, team_item[0]))

        if frame_idx % 500 == 0:
            print(f"    处理到帧 {frame_idx}, 当前 tracklet 数 {len(tracklets)}", flush=True)
...
def stage2_global_reassoc(tracklets, total_frames, args):
    ...
    print("[Stage 2] 全局重关联（时间互斥约束、出场过滤、保守保留候选簇）...")
    ...
    prototypes = getattr(args, "team_prototypes", [])
    prototype_vectors = [np.asarray(row.get("feature_center", []), np.float32) for row in prototypes]
    team_labels = {}
    if prototype_vectors and all(vector.size == 198 for vector in prototype_vectors):
        centers = np.stack(prototype_vectors)
        for track_id in ids:
            feature = tracklets[track_id].get("team_feature")
            if feature is None or not np.any(feature):
                feature = aggregate_features([row[1] for row in tracklets[track_id].get("team_feature_samples", [])])
            if not np.any(feature):
                continue
            distances = np.linalg.norm(centers - feature, axis=1)
            order = np.argsort(distances)
            margin = float(distances[order[1]] - distances[order[0]]) if len(order) > 1 else 1.0
            if margin >= float(getattr(args, "team_prototype_margin", .03)):
                team_labels[track_id] = int(prototypes[int(order[0])]["cluster_id"])
        print(f"    队色原型约束: {len(team_labels)}/{len(ids)} 条轨迹获得高置信队别")
    ...
    for a in range(len(ids)):
        for b in range(a + 1, len(ids)):
            ...
            if fa & fb:
                continue
            if ta in team_labels and tb in team_labels and team_labels[ta] != team_labels[tb]:
                continue
            ea, ca = feats[ta]
            eb, cb = feats[tb]
            ...
            pair_scores[(min(ta, tb), max(ta, tb))] = sim
    ...
    root_teams = {t: ({team_labels[t]} if t in team_labels else set()) for t in ids}
    root_members = {t: {t} for t in ids}
    merge_rows = []
    rejected_incomplete_link = 0
    ...
    for sim, a, b in pairs:
        ...
        if root_frames[ra] & root_frames[rb]:
            continue
        if root_teams[ra] and root_teams[rb] and root_teams[ra] != root_teams[rb]:
            continue
        # Require every cross-cluster member pair to pass independently. This
        # blocks transitive A~B~C chains when A and C are not compatible.
        cross_scores = []
        complete_link_ok = True
        for member_a in root_members[ra]:
            for member_b in root_members[rb]:
                member_score = pair_scores.get((min(member_a, member_b), max(member_a, member_b)))
                if member_score is None or member_score < args.merge_floor:
                    complete_link_ok = False
                    break
                cross_scores.append(member_score)
            if not complete_link_ok:
                break
        if not complete_link_ok:
            rejected_incomplete_link += 1
            continue
        if len(root_frames[ra]) < len(root_frames[rb]):
            ra, rb = rb, ra
        before_a = sorted(root_members[ra])
        before_b = sorted(root_members[rb])
        parent[rb] = ra
        root_frames[ra] |= root_frames[rb]
        root_teams[ra] |= root_teams[rb]
        root_members[ra] |= root_members[rb]
        merge_rows.append({
            "pair_score": float(sim),
            "complete_link_min_score": float(min(cross_scores)),
            "left_members": before_a,
            "right_members": before_b,
            "result_members": sorted(root_members[ra]),
        })
        merged += 1
        n_clusters -= 1
    ...
    # --- (e) 出场过滤：包装层默认保留所有达标簇；max_ids仅供可选兼容模式使用。---
    stats = []
    ...
    for ratio, cnt, r in stats:
        if ratio < args.min_presence_ratio:
            continue
        if not getattr(args, "keep_all_clusters", False) and len(kept_roots) >= args.max_ids:
            break
        kept_roots.append(r)
    ...
    args._merge_audit = {
        "policy": "complete_link_over_original_tracklet_pairs",
        "parameters": {
            "max_ids_lower_bound": args.max_ids,
            "merge_floor": args.merge_floor,
            "appearance_weight": args.wa,
            "colour_weight": args.wc,
            "colour_min": args.color_min,
            "reid_max_iou": args.reid_max_iou,
        },
        "counts": {
            "input_tracklets": len(all_ids),
            "eligible_tracklets": len(ids),
            "reid_samples": int(sum(tracklets[t].get("emb_cnt", 0) for t in all_ids)),
            "reid_overlap_skips": int(sum(tracklets[t].get("reid_overlap_skips", 0) for t in all_ids)),
            "candidate_pairs": len(pairs),
            "accepted_merges": merged,
            "complete_link_rejections": rejected_incomplete_link,
            "clusters_before_presence_filter": len(cluster_frames),
            "kept_global_ids": len(kept_roots),
        },
        "merges": merge_rows,
    }
    return local2global
...
# ============================================================
def assign_team_colors(local2global, tracklets, n_clusters=3):
    gid_col = {}
    ...
    for t, g in local2global.items():
        ...
        tmp[g][1] += 1
    gids = list(tmp.keys())
    n_clusters = max(1, min(int(n_clusters), len(gids)))
    if n_clusters < 2:
        return {g: (0, 200, 0) for g in gids}
    mat = np.stack([tmp[g][0] / max(tmp[g][1], 1) for g in gids]).astype(np.float32)
    crit = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 20, 1.0)
    _, labels, _ = cv2.kmeans(mat, n_clusters, None, crit, 5, cv2.KMEANS_PP_CENTERS)
    palette = (
        [(0, 210, 255), (245, 245, 245)]
        if n_clusters == 2
        else [(0, 0, 230), (230, 120, 0), (0, 200, 200)]
    )
...
def stage3_render_and_mot(args, detections, local2global, tracklets):
    ...
    gid_col = assign_team_colors(
        local2global, tracklets, getattr(args, "team_clusters", 3)
    )

    by_frame = defaultdict(list)
    global_frame_keys = set()
    for (f, tid, x, y, w, h, c) in detections:
        if tid in local2global:
        ...
        if tid in local2global or getattr(args, "render_unassigned", False):
    ...
    render_total = max(1, int(cap.get(cv2.CAP_PROP_FRAME_COUNT)) // max(1, args.vid_stride))
    while True:
        ret, frame = cap.read()
        if not ret:
            break
        ...
        for (tid, x, y, w, h, c) in by_frame.get(proc_idx, []):
            g = local2global.get(tid)
            col = gid_col.get(g, (130, 130, 130))
            cv2.rectangle(frame, (int(x), int(y)), (int(x + w), int(y + h)), col, 2)
            label = f"ID {g}" if g is not None else f"T {tid}"
        ...
        writer.write(frame)
        if proc_idx % 300 == 0:
            print(f"    渲染到帧 {proc_idx}, 总帧 {render_total}", flush=True)
    cap.release()
    writer.release()
...
def validate_args(ap, args):
    ...
    if not 0 <= args.reid_max_iou <= 1:
        ap.error("--reid_max_iou 必须在 0..1")
    if args.max_ids < 1:
...
def main():
    ...
    ap.add_argument("--reid_max_iou", type=float, default=0.40,
                    help="人物框重叠达到该值时不采集全局关联外观")
    # 全局重关联 + 出场过滤
    ap.add_argument("--max_ids", type=int, default=35,
                    help="最终全局 ID 上限")
    ap.add_argument("--min_track_frames", type=int, default=1,
                    help="合并前剔除出场少于该帧数的碎片 tracklet")
    ap.add_argument("--min_presence_ratio", type=float, default=0.0,
