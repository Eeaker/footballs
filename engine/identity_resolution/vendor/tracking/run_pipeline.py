...
OPTIONAL_TRACKING_FILES = (
    "metric_positions.csv", "metric_motion_summary.json", "tracking_local_mot.txt",
    "tracking_local_mot_pre_metric_gate.txt", "local_to_global.json",
    "global_merge_audit.json", "metric_field_gate_report.json",
)
...
def main() -> None:
    ...
    with (output / "pipeline.log").open("w", encoding="utf-8") as log:
        tracking_command = [
            ...
            "--min_track_frames", str(get_value(config, "identity.min_track_frames", 1)),
            "--min_presence_ratio", str(get_value(config, "identity.min_presence_ratio", 0.0)),
        ...
        if bool(get_value(config, "metric_field_gate.enabled", False)):
            calibration_path = resolve_config_path(config, "metric_field_gate.calibration_path")
            if calibration_path is None or not calibration_path.is_file():
                raise FileNotFoundError("metric field gate calibration not found")
            tracking_command.extend([
                "--metric-field-calibration", str(calibration_path),
                "--metric-field-margin-m", str(get_value(config, "metric_field_gate.association_margin_m", 0.0)),
                "--metric-field-hard-margin-m", str(get_value(config, "metric_field_gate.hard_outside_margin_m", 0.0)),
                "--metric-field-min-track-ratio", str(get_value(config, "metric_field_gate.minimum_track_inside_ratio", .5)),
                "--metric-field-min-frames", str(get_value(config, "metric_field_gate.minimum_track_inside_frames", 10)),
            ])
