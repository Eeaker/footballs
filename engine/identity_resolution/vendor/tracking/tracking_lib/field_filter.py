...
def restrict_tracklet_frames(tracklets: dict, detections: Iterable[tuple]) -> None:
    ...
    for track_id, tracklet in tracklets.items():
        ...
        if "appearance_samples" in tracklet:
            appearance = [sample for sample in tracklet["appearance_samples"] if sample[0] in accepted]
            tracklet["appearance_samples"] = appearance
            if appearance:
                tracklet["emb_sum"] = np.sum([sample[1] for sample in appearance], axis=0).astype(np.float32)
                tracklet["emb_cnt"] = len(appearance)
                tracklet["col_sum"] = np.sum([sample[2] for sample in appearance], axis=0).astype(np.float32)
                tracklet["col_cnt"] = len(appearance)
            else:
                tracklet["emb_sum"] = np.zeros_like(tracklet["emb_sum"])
                tracklet["emb_cnt"] = 0
                tracklet["col_sum"] = np.zeros_like(tracklet["col_sum"])
                tracklet["col_cnt"] = 0
